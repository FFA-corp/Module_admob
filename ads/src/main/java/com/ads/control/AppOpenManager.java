...
//        if (!Purchase.getInstance().isRemoveAds(application.getApplicationContext()) &&
...
//        }
    ...
    public void showAdIfAvailable(final boolean isSplash) {
        ...
        if (currentActivity != null && Purchase.getInstance().isRemoveAds(currentActivity)) {
...
//            }
    ...
    public void loadSplashAd(final String adId) {
        splashAdId=adId;
        isTimeout = false;
        if (currentActivity != null && Purchase.getInstance().isRemoveAds(currentActivity)) {
            if (fullScreenContentCallback != null) {
